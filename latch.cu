...
__forceinline__ __device__ void patchComparison(volatile float s_roi[roiWidth][roiWidth + roiWidthPadding],
...
__global__ void __launch_bounds__(1024, 2)
                latch(  const unsigned char *g_img,
                        const int *g_K,
                        unsigned int *g_D,
                        const int imgWidth,
                        const int imgHeight) {
    ...
    volatile __shared__ unsigned int s_out[warpsPerBlock];
    {
        ...
        { // Give everyone the global memory offset to the bottom left corner of the region of interest. (Assuming y-up, row-major.)
            if (threadIdx.y == 0 && threadIdx.x < 2) { // 2 threads, 2 coordinates
                ...
                if (threadIdx.x == 1) {
                    if (imgHeight-2*_warpSize < k) {
                        k = -999999; // Guards against top border. (assuming y-up is positive, in contrast to graphics conventions)
                    } else {
                        k *= imgWidth;
                    }
                }
            ...
            __threadfence_block();
            __syncthreads();
            if (threadIdx.y != 0 && threadIdx.x == 0) { // Faster to have half warp grab, then broadcast
                kpOffset = s_kpOffset;
            }
        ...
        s_roi[threadIdx.y            ][threadIdx.x            ] = (float) g_img[kpOffset + (            threadIdx.y)*imgWidth + (            threadIdx.x)];
        s_roi[threadIdx.y            ][threadIdx.x + _warpSize] = (float) g_img[kpOffset + (            threadIdx.y)*imgWidth + (_warpSize + threadIdx.x)];
        s_roi[threadIdx.y + _warpSize][threadIdx.x            ] = (float) g_img[kpOffset + (_warpSize + threadIdx.y)*imgWidth + (            threadIdx.x)];
        s_roi[threadIdx.y + _warpSize][threadIdx.x + _warpSize] = (float) g_img[kpOffset + (_warpSize + threadIdx.y)*imgWidth + (_warpSize + threadIdx.x)];
    }
    ...
    const register float mask0 = 1.0f;
    const register float mask1 = 1.0f;
    __syncthreads();
    __threadfence_block();

    #pragma unroll
    for (register int i=0; i<16; ) {
        const register int coord = tex2D(patchTriplets, 6*i + threadIdx.x, threadIdx.y); // This access is hardware cached.
        #pragma unroll
        for (register int j=0; j<4; j++, i++) {
            patchComparison(s_roi, __shfl(coord, 6*j  ),
                            __shfl(coord, 6*j+1),
                            __shfl(coord, 6*j+2),
                            __shfl(coord, 6*j+3),
                            __shfl(coord, 6*j+4),
                            __shfl(coord, 6*j+5),
                            wrappedX, wrappedY,  16*(threadIdx.y & 1) + i,  0, &out, mask0, mask1);
        }
    }
...
void loadPatchTriplets(cudaArray* cuArray) {
    ...
    cudaMemcpyToArray(cuArray, 0, 0, h_data, 6*16*32*sizeof(int), cudaMemcpyHostToDevice);
...
void latch(unsigned char* img, int* h_K, unsigned int* d_D, int* keypoints, int maxKP, int* d_K, unsigned char* d_img, vector<KeyPoint>* vectorKP, const int imgWidth, const int imgHeight, cudaEvent_t latchFinished) {
